<?php
  ...
  // $d = mysqli_query($con, $query4);
	...
	$query2 = "SELECT s.c_id AS c_id,s.year AS year, s.semester AS semester, s.q_b_link AS link, a.q_b_img AS img
		FROM question_bank AS s JOIN question_bank_img AS a ON (s.q_b_no=a.q_b_no)
		WHERE s.c_id='$key' and s.year='$year';";
   $e = mysqli_query($con, $query2);

	 while($row = mysqli_fetch_assoc($e)) {
     $questions[$row['c_id']] = array( 'year' => $row['year'], 'semester' => $row['semester'],'img' => $row['img'], 'link'=> $row['link'] );
      ...
      <div class="collapse navbar-collapse">
        <ul class="navbar-nav ml-auto">
          ...
          <li class="button-container nav-item iframe-extern">
            <a href="add_ques_bank.php?key=<?php echo $key; ?>" target="_blank" class="btn  btn-rose   btn-round btn-block">
             Upload New Question
            </a>
          </li>
        </ul>
 ...
 -->
  ...
  <div class="box">
    <div class="box1">
      ...
      <a href="questionsbank.php?key=<?php echo $key;?>"class="questions">Question Bank</a>
      ...
      <!-- <div class="sorted">
        <div class="second">
          <button>
			...
			<table>
				<?php foreach ($questions as $c_id => $question) {
				?>
					...
					<tr>
						<td>
							...
							<div class="question">
								...
								<?php echo $key;?>&nbsp;<b><?php echo $question['semester']; ?>&nbsp;&nbsp;<?php echo $question['year']; ?></b><br><br>
								<a href="Ques_uploads/<?php echo $question['img'];?>">Click to download </a>
								<?php echo 'Download Link : '; ?><?php echo $question['link']; ?>
							</div>
	      			</td>
	      	</tr>
