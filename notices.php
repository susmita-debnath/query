<?php
	...
	$notice_no = $_REQUEST['notice_no'];
	  ...
	  $query1 = "SELECT * FROM course WHERE c_id != '$key'";
  	...
  	$query = "SELECT * from notice where notice_no = '$notice_no';";
...
<body>
  ...
  <div class="box">
		<div class="box1">
      <a href="index.php" class="courses">My Courses</a>
      <table>
        <?php while($row = mysqli_fetch_assoc($a) ) {
        ?>
				<tr>
          <td><a href="subject.php?key=<?php echo $row['c_id']?>"><b><?php echo $row['c_id'] ?></b><br> <?php echo $row['c_name'] ?><br><br></a></td>
        </tr>
        <?php } ?>
      </table>
    </div>
    <div class="box2">
      <div class="answers:"><h3>Notices</h3></div><br><br>
      <table class="table">
      	<?php while($rows = mysqli_fetch_assoc($s)) {
      	?>

      	<tr class="answer_box">
					<?php str_replace("\r","<li></li>",$row['body']);?>
      			<td><div class="avatar"><b><?php echo $rows['header']; ?></b><br><br>
      				<?php echo $rows['body']; ?></div>
      			</td>
      	</tr>
